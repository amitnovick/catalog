...
import { updateRelationship } from '../../../../sql_queries';
...
const createRelationship = async (supercategoryName, subcategoryName) => {
  ...
  return new Promise(async (resolve, reject) => {
    if (subcategoryName === rootCategory.name || subcategoryName.trim() === '') {
      reject();
    } else {
      getSqlDriver().run(
        updateRelationship,
...
const machineWithServices = machine.withConfig({
  services: {
    ...
    createRelationship: (_, event) => createRelationship(event.parentTag, event.childTag),
  },
});
...
const CategoriesPanel = () => {
  ...
  const addNewCategory = (categoryName) => send('ADD_NEW_CATEGORY', { tag: categoryName });
  ...
  if (['loadingAddingNewCategory', 'loadingCreatingRelationship'].includes(current.value)) {
    ...
    return (
      <React.Fragment>
        ...
        <CreateSubcategoryRelationshipContainer onClickCreateRelationship={createRelationship} />
